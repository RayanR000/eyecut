...
#
# `import-ass` is the only one left: `caption` and `tts` are refused at
# validation now, so nothing reaches `apply_post_ops` for them.


def test_an_import_ass_maps_its_options_onto_the_cli(tmp_path):
    """The op owns the whole shape -- compile has no vocabulary for it -- so an
    unmapped option is a draft that opens looking like one nobody asked to
    change."""
    calls = []
    warnings = apply_post_ops(
        {"operations": [{"op": "import-ass", "path": "/s/a.ass",
                         "trackName": "subtitle", "fontSize": 20}]},
        tmp_path, runner=lambda argv, cwd: (calls.append(argv) or (0, "")),
        store=tmp_path)

    assert warnings == []
    assert calls[0][:4] == ["capcut", "import-ass", str(tmp_path), "/s/a.ass"]
    assert calls[0][calls[0].index("--track-name") + 1] == "subtitle"
...
def test_a_failing_post_op_warns_and_keeps_the_draft(tmp_path):
    ...
    warnings = apply_post_ops(
        {"operations": [{"op": "import-ass", "path": "/s/a.ass"}]},
        tmp_path, runner=lambda argv, cwd: (1, "no such file"),
        store=tmp_path)

    assert len(warnings) == 1 and "import-ass" in warnings[0]
